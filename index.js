 ...
 * ║                         CHANGE LOG                                       ║
 * ╠═══════════════════════════════════════════════════════════════════════════╣
 * ║ v6.4.0-fp4 (2026-02-26) — SAVE PROFILE ARGUMENT                         ║
 * ║                                                                           ║
 * ║   [NEW] --save argument: saves persistent profile to ./profile           ║
 * ║         - Without --save: temp profile in os.tmpdir(), auto-deleted      ║
 * ║         - With --save: profile saved to ./profile, survives exit         ║
 * ║         - If ./profile exists: loads existing profile (reuse session)    ║
 * ║         - If ./profile not exists: creates new directory                 ║
 * ║         - --persist=<dir> takes precedence over --save                   ║
 * ║   [CHG] CLI parsing: added saveProfile boolean from args.includes       ║
 * ║   [CHG] runScan() profile logic: added else-if branch for saveProfile   ║
 * ║   [CHG] Banner output: shows --save / profile path status               ║
 * ║   [CHG] Usage help: added --save option description                     ║
 * ║   [NOTE] Touch points verified unchanged (no modification needed):      ║
 * ║     - Post-scan cleanup (isAutoGenerated=false prevents cleanup)         ║
 * ║     - cleanupTempDirs() (--save dirs never in cleanup set)              ║
 * ║     - SIGINT/SIGTERM (only cleans temp dirs, --save survives)           ║
 * ║     - injectionStatus (uses variables, auto-correct)                     ║
 * ╠═══════════════════════════════════════════════════════════════════════════╣
 ...
 * ╚═══════════════════════════════════════════════════════════════════════════╝
 *
 * Sentinel v6.4.0-fp4 — Persistent Fingerprint Injection
 *
 * PERSISTENCE MODEL:
 ...
 *
 * PROFILE MODES:
 *   Default (no flag)  → temp profile in os.tmpdir(), auto-deleted after scan
 *   --save             → profile saved to ./profile, reused on next run
 *   --persist=<dir>    → profile at custom path, not auto-deleted
 *
...
const VERSION = 'sentinel-v6.4.0-fp4';

const FINGERPRINT_PATH = path.join(__dirname, 'fingerprint.json');
const CHROME_PATH = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';
const SAVE_PROFILE_DIR = path.join(__dirname, 'profile');
...
const saveProfile = args.includes('--save');
...
Options:
  ...
  --wait=<ms>        Post-load wait time (default: 60000)
  --save             Save browser profile to ./profile for reuse
                     If ./profile exists, loads existing profile
                     If ./profile not exists, creates new profile
  --persist=<dir>    Custom persistent profile directory (overrides --save)

Examples:
  node index.js https://browserscan.net --no-headless
    (temp profile, auto-deleted after scan)

  node index.js https://browserscan.net --no-headless --save
    (profile saved to ./profile, reused on next run)

  node index.js https://browserscan.net --persist=./my-profile --no-headless
    (custom profile path, not auto-deleted)
`);
  process.exit(0);
}
...
async function runScan(mode) {
  ...
  if (userPersistDir) {
    ...
    persistDir = SAVE_PROFILE_DIR;
    fs.mkdirSync(persistDir, { recursive: true });
    if (fs.readdirSync(persistDir).length > 0) {
      console.log(`[Sentinel] Loading existing saved profile: ${persistDir}`);
    } else {
      console.log(`[Sentinel] Creating new saved profile: ${persistDir}`);
    }
  } else {
...
// ─── Main ───
(async () => {
  const profileDisplay = userPersistDir
    ? `custom: ${userPersistDir}`
    : saveProfile
      ? `saved: ${SAVE_PROFILE_DIR}`
      : 'auto-generated temp (with cleanup)';
