...
/**
 * ╔══════════════════════════════════════════════════════════════╗
 * ║   SENTINEL v4.2 — FORENSIC MALING CATCHER                   ║
  ...
  try {
    ...
    for (const script of scripts) {
      await cdpSession.send('Page.addScriptToEvaluateOnNewDocument', {
        ...
        // runImmediately: true — runs before DOM parsing
        runImmediately: true
      });
    }
    ...
    try {
      ...
      cdpSession.on('Target.attachedToTarget', async function(params) {
        ...
        try {
          // Inject into attached target via its session
          for (const script of scripts) {
            await cdpSession.send('Page.addScriptToEvaluateOnNewDocument', {
              source: '/* SENTINEL_L3 */ window.__SENTINEL_L3__ = true;\n' + script,
              runImmediately: true
            }, sessionId);
          }
          ...
          // Some targets may not support Page domain
          try {
            await cdpSession.send('Runtime.runIfWaitingForDebugger', {}, sessionId);
          } catch(e2) {}
        }
...
async function runScan(url, options) {
  ...
  console.log('\n' + '═'.repeat(65));
  console.log('  ' + label + ' MODE — Sentinel v4.2 Forensic Scan');
  ...
  try {
    ...
    // ── Merge push telemetry from CDP binding ──
    if (injectionResult && injectionResult.pushEvents.length > 0) {
      console.log('  📡 Push telemetry: ' + injectionResult.pushEvents.length + ' events');
      sentinelData.events = (sentinelData.events || []).concat(injectionResult.pushEvents);
    }

    // Merge grace period events
    if (graceData && graceData.events && graceData.events.length > 0) {
      sentinelData.events = (sentinelData.events || []).concat(graceData.events);
    }
    ...
    console.log('┌──────────────────────────────────────────────────────┐');
    console.log('│  🛡️ SENTINEL v4.2 FORENSIC SUMMARY                   │');
    console.log('├──────────────────────────────────────────────────────┤');
...
(async function() {
  console.log('\n  ╔══════════════════════════════════════════════════════════════╗');
  console.log('  ║   🛡️  SENTINEL v4.2 — FORENSIC MALING CATCHER               ║');
