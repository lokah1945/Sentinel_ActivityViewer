// ═══════════════════════════════════════════════════════════════
//  SENTINEL v5.0.0 — UNIFIED REPORT GENERATOR
//  Contract: C-RPT-01 through C-RPT-10
//  JSON + HTML (dark theme) + CTX reports
//  Fix: vc is not defined (v4.5 bug) — strict scoping
//  Fix: timeSpanMs uses Math.max/reduce — never 0
// ═══════════════════════════════════════════════════════════════
...
function generateHTML(report, networkConv, targetGraph, valueCaptures, correlation) {
  ...
  return '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">' +
    '<title>Sentinel v5.0.0 — ' + escHtml(report.target) + '</title>' +
    '<style>' + getCSS() + '</style></head><body>' +
    '<div class="container">' +
    '<header><h1>🛡️ Sentinel v5.0.0 — Unified Forensic Engine</h1>' +
    ...
    '<footer><p>Sentinel v5.0.0 Unified Forensic Engine — Generated ' + new Date().toISOString() + '</p></footer>' +
    '</div></body></html>';
}
