...
function generateHtml(report, correlation) {
  ...
  var catRows = Object.entries(report.byCategory)
    .sort(function(a, b) { return b[1] - a[1]; })
    .map(function(x) {
      return '<tr><td>' + (vc.ts/1000).toFixed(2) + 's</td><td>' + (vc.dir === 'response' ? '📤 answer' : '📥 call') + '</td><td>' + vc.api + '</td><td>' + vc.category + '</td><td style="word-break:break-all;max-width:400px">' + (vc.value || '-').replace(/</g,'&lt;') + '</td></tr>';
    }).join('');
  ...
  return '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">' +
    ...
    (function() {
      ...
      return '<div class="section"><h2>🔄 Network Conversation (' + nc.requests + ' req / ' + nc.responses + ' resp)</h2>' +
